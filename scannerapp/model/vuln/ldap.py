...
# nmap -sU -p 123 --script ntp-info <target>
    ...
    def parseAsCustom(self, result):
        ...
        if self.isVulnerable(result):
            v.append({"address":self.getAddress(),"ports":self.getDefaultPorts(),"evidence":result})
        else:
            notv.append({"address":self.getAddress(),"ports":self.getDefaultPorts(),"evidence":result})
